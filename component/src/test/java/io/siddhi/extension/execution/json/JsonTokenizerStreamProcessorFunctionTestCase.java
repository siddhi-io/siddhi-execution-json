...
public class JsonTokenizerStreamProcessorFunctionTestCase {
    private static final Logger log = LogManager.getLogger(JsonTokenizerStreamProcessorFunctionTestCase.class);
    ...
    @Test
    public void testJsonTokenizerWithStringInput() throws InterruptedException {
                ...
                for (Event event : inEvents) {
                    ...
                    switch (count.get()) {
                        ...
                        case 6:
                            ...
                            break;
                        case 7:
                            AssertJUnit.assertEquals("[{\"barName\":\"barName3\"},{\"barName\":\"barName4\"}]", event
                                    .getData(0));
                            break;
                    }
                }
    ...
    public void testJsonTokenizerWithObjectInput() throws InterruptedException, ParseException {
        log.info("JsonTokenizerStreamProcessorFunction - testJsonTokenizerWithObjectInput");
        SiddhiManager siddhiManager = new SiddhiManager();
        String stream = "define stream InputStream(json object,path string);\n";
        String query = ("@info(name = 'query1')\n" +
                "from InputStream#json:tokenize(json, path, false)\n" +
                "select jsonElement\n" +
                "insert into OutputStream;");
        SiddhiAppRuntime siddhiAppRuntime = siddhiManager.createSiddhiAppRuntime(stream + query);
        siddhiAppRuntime.addCallback("query1", new QueryCallback() {
